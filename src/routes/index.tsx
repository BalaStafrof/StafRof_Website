  ...
  return (
    <>
      ...
      <section className="hero-cosmic min-h-[92vh] pt-32 pb-24 flex items-center relative">
        ...
        <div className="relative w-full max-w-[1400px] mx-auto px-4 sm:px-8 lg:px-16 z-10 pointer-events-none">
            ...
            <Reveal delay={150} direction="right" className="lg:col-span-5 w-full flex justify-center items-center">
              <div className="relative w-full overflow-hidden rounded-3xl border border-white/10 bg-white/[0.02] p-2 shadow-[0_24px_50px_-15px_rgba(0,0,0,0.5)] ring-1 ring-white/10 group">
                ...
                <img
                  ...
                  alt="Who We Are - Stafróf technology"
                  className="w-full h-auto object-contain rounded-2xl relative z-10 transition-transform duration-700 group-hover:scale-[1.02]"
                  loading="lazy"
      ...
      {/* === OUR PURPOSE === */}
      <section className="py-24 bg-gradient-to-br from-[#020b2d] via-[#091e5c] to-[#0091ff] text-white border-b border-blue-900/30 relative overflow-hidden">
      ...
      <section className="animated-gradient-bg relative overflow-hidden py-24 sm:py-32">
        ...
        <div className="relative mx-auto max-w-6xl px-6">
          <Reveal className="text-center mb-16">
            <p className="eyebrow-dark">Next-Gen Platforms</p>
            <h2 className="mt-3 font-display text-4xl font-bold text-white sm:text-6xl tracking-tight">
              RevNexure, DataXentra &amp; AgenNova
            </h2>
          </Reveal>

          {/* Document Inline Images for Platforms in a vertical stack for maximum size and readability */}
          <div className="mt-12 space-y-16">
            {[
              { img: DOC_IMAGES.image3, name: "RevNexure 360° Suite", desc: "Flagship AI-powered Revenue Assurance Platform providing comprehensive auditing, leakage prevention, and recovery mechanisms across all core banking channels." },
              { img: DOC_IMAGES.image4, name: "DataXentra Platform", desc: "Enterprise Data Platform that unifies data storage, automates reconciliation pipelines, and enforces strict governance to build a trusted BI foundation." },
              { img: DOC_IMAGES.image5, name: "AgenNova AI Platform", desc: "AI Intelligence Platform powering autonomous agents, automated back-office workflows, and intelligent decision systems." }
            ].map((item, i) => (
              <Reveal key={i} delay={i * 150} className="w-full flex flex-col items-center">
                <div className="w-full max-w-5xl group relative overflow-hidden rounded-3xl border border-blue-500/15 bg-white/[0.01] p-6 shadow-[0_24px_50px_-15px_rgba(0,0,0,0.5)] hover:border-blue-400/40 hover:bg-white/[0.02] transition-all duration-300">
                  <div className="absolute inset-0 bg-gradient-to-tr from-brand/5 via-transparent to-[#00C2FF]/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none rounded-3xl" />
                  
                  <div className="mb-6 text-center sm:text-left">
                    <h3 className="font-display text-2xl font-bold text-white group-hover:text-brand transition-colors duration-200">{item.name}</h3>
                    <p className="mt-2 text-sm text-white/70 max-w-3xl leading-relaxed">{item.desc}</p>
                  </div>
                  
                  <div className="overflow-hidden rounded-2xl bg-black/30 p-4 flex justify-center items-center">
                    <img
                      src={item.img}
                      alt={item.name}
                      className="w-full h-auto max-h-[600px] object-contain rounded-xl transition-transform duration-500"
                      loading="lazy"
                  ...
                  <div className="bg-gradient-to-b from-[#061233]/95 to-[#030a1f]/98 backdrop-blur-xl border border-blue-500/25 p-6 sm:p-8 rounded-3xl shadow-2xl text-left">
                    ...
                    <form onSubmit={(e) => {
                      ...
                      .then(res => {
                        ...
                        toast.success("Thank you! Your inquiry has been sent to business@stafrof.com.");
                        target.reset();
                      })
                      .catch(() => {
                        toast.error("Failed to submit. Please try again or email business@stafrof.com directly.");
                      });
                    }} className="space-y-4">
                      <div>
                        <label className="block text-xs font-semibold text-white/80 uppercase tracking-wider mb-1">Name *</label>
                        <input name="name" type="text" required className="w-full bg-white/[0.05] border border-white/10 rounded-lg px-4 py-2.5 text-white placeholder-white/40 focus:outline-none focus:border-brand transition-colors text-sm" placeholder="Your name" />
                      </div>
                      
                      <div>
                        <label className="block text-xs font-semibold text-white/80 uppercase tracking-wider mb-1">Company *</label>
                        <input name="company" type="text" required className="w-full bg-white/[0.05] border border-white/10 rounded-lg px-4 py-2.5 text-white placeholder-white/40 focus:outline-none focus:border-brand transition-colors text-sm" placeholder="Company name" />
                      </div>

                      <div className="grid gap-4 sm:grid-cols-2">
                        <div>
                          <label className="block text-xs font-semibold text-white/80 uppercase tracking-wider mb-1">Email *</label>
                          <input name="email" type="email" required className="w-full bg-white/[0.05] border border-white/10 rounded-lg px-4 py-2.5 text-white placeholder-white/40 focus:outline-none focus:border-brand transition-colors text-sm" placeholder="Business email" />
                        </div>
                        <div>
                          <label className="block text-xs font-semibold text-white/80 uppercase tracking-wider mb-1">Phone *</label>
                          <input name="phone" type="tel" required className="w-full bg-white/[0.05] border border-white/10 rounded-lg px-4 py-2.5 text-white placeholder-white/40 focus:outline-none focus:border-brand transition-colors text-sm" placeholder="Phone number" />
                        </div>
                      </div>

                      <div>
                        <label className="block text-xs font-semibold text-white/80 uppercase tracking-wider mb-1">Message *</label>
                        <textarea name="message" required rows={4} className="w-full bg-white/[0.05] border border-white/10 rounded-lg px-4 py-2.5 text-white placeholder-white/40 focus:outline-none focus:border-brand transition-colors text-sm resize-none" placeholder="Your message..." />
                      </div>

                      <button type="submit" className="w-full premium-btn premium-btn-hover justify-center text-sm font-semibold uppercase tracking-wider py-3 mt-2">
                        Submit Enquiry
                      </button>
                    </form>
                  </div>
